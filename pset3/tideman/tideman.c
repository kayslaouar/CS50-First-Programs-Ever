...
bool check_cycles(pair curr_pair, pair locked_pairs_ref[], int arr_length);
bool check_cycles2(pair curr_pair, pair locked_pairs_ref[], int arr_length);
...
void lock_pairs(void)
{
    ...
    {
        if (!check_cycles(pairs[i], locked_pairs, locked_pairs_length))
        {
            if (!check_cycles2(pairs[i], locked_pairs, locked_pairs_length))
            {
...
bool check_cycles(pair curr_pair, pair locked_pairs_ref[], int arr_length)
{
    // checks if current loser has ever beat the current winner or anyone that beats the current winner
    if (locked[curr_pair.loser][curr_pair.winner])
    {
        return true;
    }
    else
    {
        ...
        {
            if (curr_pair.loser == locked_pairs_ref[i].winner)
            {
                curr_pair.loser = locked_pairs_ref[i].loser;
                return check_cycles(curr_pair, locked_pairs_ref, arr_length);
            }
        }
    }
    return false;
...
bool check_cycles2(pair curr_pair, pair locked_pairs_ref[], int arr_length)
{
    // checks if current winner has ever lost to someone that beats the current winner
    if (locked[curr_pair.loser][curr_pair.winner])
    {
        return true;
    }
    else
    {
        for (int i = 0; i < arr_length; i++)
        {
            if (curr_pair.winner == locked_pairs_ref[i].loser)
            {
                curr_pair.winner = locked_pairs_ref[i].winner;
                return check_cycles2(curr_pair, locked_pairs_ref, arr_length);
            }
        }
    }
    return false;
