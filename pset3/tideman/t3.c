...
bool final_check(struct curr_pair, struct pair_locked[]);
...
bool final_check(pair curr_pair, pair pair_locked[])
{
    bool cycle = false;
    for (int i = 0; i < length; i++)
    {
        int winners_loss = -1;
        // lower index means curr_winner lost early in locked pairs. If earlier than curr_winner, then there is a cycle
        int losers_win = -1;
        // lower index means curr_loser won early in locked pairs. If earlier that curr_winner, then there is a cycle
        if (locked_pairs[i].winner == curr_pair.loser)
        {
            losers_win = i;
        }
        if (locked_pairs[i].loser == curr_pair.winner)
        {
            winners_loss = i;
        }
    }
    cycle = winners_loss <= losers_win || losers_win <= winners_loss)
    if (cycle)
    {
        return true;
    }
